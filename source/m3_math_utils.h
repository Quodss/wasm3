...
static const f32 f32_SMALL_ILLEGAL_u32 = _f32_neg_one;  // -1
static const f32 f32_SMALL_ILLEGAL_i32 = (
    (u32)1 << 31
    | ( (f32)(f32_BIAS + 31) << f32_LEN_MANT )
    | ( (f32)1U )
);         // -1 * (2^(N-1) + epsilon)

static const f32 f32_SMALL_ILLEGAL_u64 = _f32_neg_one;  
static const f32 f32_SMALL_ILLEGAL_i64 = (
    (u32)1 << 31
    | ( (f32)(f32_BIAS + 63) << f32_LEN_MANT )
    | (f32) 1U
);  // -1 * (2^(N-1) + epsilon)

static const f64 f64_SMALL_ILLEGAL_u32 = _f64_neg_one;  
static const f64 f64_SMALL_ILLEGAL_i32 = (
    (u64)1 << 63
    | ( (f64)(f64_BIAS + 31) << f64_LEN_MANT )
    | ( (f64)1U << (f64_LEN_MANT - 31) )
);  // -1 * (2^(N-1) + 1)

static const f64 f64_SMALL_ILLEGAL_u64 = _f64_neg_one;  
static const f64 f64_SMALL_ILLEGAL_i64 = (
    (u64)1 << 63
    | ( (f64)(f64_BIAS + 63) << f64_LEN_MANT )
    | ( (f64)1U )
);  // -1 * (2^(N-1) + epsilon)

static const f32 f32_BIG_ILLEGAL_u32 = ((u32)f32_BIAS + 32) << f32_LEN_MANT;  // 2^N
static const f32 f32_BIG_ILLEGAL_i32 = ((u32)f32_BIAS + 31) << f32_LEN_MANT;  // 2^(N-1)

static const f32 f32_BIG_ILLEGAL_u64 = ((u32)f32_BIAS + 64) << f32_LEN_MANT;
static const f32 f32_BIG_ILLEGAL_i64 = ((u32)f32_BIAS + 63) << f32_LEN_MANT;

static const f64 f64_BIG_ILLEGAL_u32 = ((u64)f64_BIAS + 32) << f64_LEN_MANT;
static const f64 f64_BIG_ILLEGAL_i32 = ((u64)f64_BIAS + 31) << f64_LEN_MANT;

static const f64 f64_BIG_ILLEGAL_u64 = ((u64)f64_BIAS + 64) << f64_LEN_MANT;
static const f64 f64_BIG_ILLEGAL_i64 = ((u64)f64_BIAS + 63) << f64_LEN_MANT;

#define SF_COMP(OP, TYPE, A, B)                                                 \
    ( TYPE##_##OP(TO_SF(TYPE, A), TO_SF(TYPE, B)) )

#define OP_TRUNC(RES, A, TO, FROM)                                              \
    if (UNLIKELY(FROM##_isnan(TO_SF(FROM, A))))                                 \
    {                                                                           \
        newTrap (m3Err_trapIntegerConversion);                                  \
    }                                                                           \
    if (UNLIKELY(                                                               \
        SF_COMP(le, FROM, A, FROM##_SMALL_ILLEGAL_##TO)                         \
        || SF_COMP(le, FROM, FROM##_BIG_ILLEGAL_##TO, A)                        \
    ))                                                                          \
    {                                                                           \
        newTrap (m3Err_trapIntegerOverflow);                                    \
    }                                                                           \
    RES = _##TO##_trunc_##FROM(A);
...
#define OP_TRUNC_SAT(RES, A, TO, FROM, IMIN, IMAX)                              \
    if (UNLIKELY(FROM##_isnan(TO_SF(FROM, A))))                                 \
    {                                                                           \
        RES = 0;                                                                \
    }                                                                           \
    else if (UNLIKELY(SF_COMP(le, FROM, A, FROM##_SMALL_ILLEGAL_##TO)))         \
    {                                                                           \
        RES = IMIN;                                                             \
    }                                                                           \
    else if (UNLIKELY(SF_COMP(le, FROM, FROM##_BIG_ILLEGAL_##TO, A)))           \
    {                                                                           \
        RES = IMAX;                                                             \
    }                                                                           \
    else                                                                        \
...
#define OP_I32_TRUNC_SAT_F32(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, i32, f32, INT32_MIN, INT32_MAX)

#define OP_U32_TRUNC_SAT_F32(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, u32, f32, (u32)0,    UINT32_MAX)

#define OP_I32_TRUNC_SAT_F64(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, i32, f64, INT32_MIN, INT32_MAX)

#define OP_U32_TRUNC_SAT_F64(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, u32, f64, (u32)0,    UINT32_MAX)


#define OP_I64_TRUNC_SAT_F32(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, i64, f32, INT64_MIN, INT64_MAX)

#define OP_U64_TRUNC_SAT_F32(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, u64, f32, (u64)0,    UINT64_MAX)

#define OP_I64_TRUNC_SAT_F64(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, i64, f64, INT64_MIN, INT64_MAX)

#define OP_U64_TRUNC_SAT_F64(RES, A)                                            \
    OP_TRUNC_SAT(RES, A, u64, f64, (u64)0,    UINT64_MAX)
...
_nan_unify(f32)                 _nan_unify(f64)


#define SF_BinaryComparative(OP, TYPE)                                          \
    static inline bool                                                          \
    _##TYPE##_##OP(TYPE a, TYPE b)                                              \
    {                                                                           \
        return SF_COMP(OP, TYPE, a, b);                                         \
    }

#define SF_Binary(OP, TYPE)                                                     \
    static inline TYPE                                                          \
    _##TYPE##_##OP(TYPE a, TYPE b)                                              \
    {                                                                           \
        softfloat_roundingMode = softfloat_round_near_even;                     \
        return _##TYPE##_nan_unify(                                             \
            OF_SF(TYPE,                                                         \
            TYPE##_##OP(TO_SF(TYPE, a), TO_SF(TYPE, b))                         \
        ));                                                                     \
    }

SF_BinaryComparative(eq, f32)   SF_BinaryComparative(eq, f64)
SF_BinaryComparative(lt, f32)   SF_BinaryComparative(lt, f64)
SF_BinaryComparative(le, f32)   SF_BinaryComparative(le, f64)

SF_Binary(add, f32)             SF_Binary(add, f64)
SF_Binary(mul, f32)             SF_Binary(mul, f64)
SF_Binary(sub, f32)             SF_Binary(sub, f64)
SF_Binary(div, f32)             SF_Binary(div, f64)
...
_trunc_convert(u32, f32)        _trunc_convert(u32, f64)
_trunc_convert(i32, f32)        _trunc_convert(i32, f64)
_trunc_convert(u64, f32)        _trunc_convert(u64, f64)
_trunc_convert(i64, f32)        _trunc_convert(i64, f64)
...
_convert(f32, u32)              _convert(f64, u32)
_convert(f32, i32)              _convert(f64, i32)
_convert(f32, u64)              _convert(f64, u64)
_convert(f32, i64)              _convert(f64, i64)
